...
function Counter({ target, prefix, suffix, color, label, inView }) {
  ...
  return React.createElement('div', { style: { textAlign: 'center', padding: '1.5rem 2rem' } },
    React.createElement('div', { style: { fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2.5rem,5vw,3.5rem)', color, lineHeight: 1, textShadow: '0 0 30px ' + color + '40', letterSpacing: 2 } },
      (prefix || '') + count + (suffix || '')),
...
export default function Home() {
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    el.style.setProperty('--mx', (e.clientX - rect.left) / rect.width - 0.5);
    el.style.setProperty('--my', (e.clientY - rect.top) / rect.height - 0.5);
  }, []);
  ...
  const CSS = `
    ...
    .cursor { display:inline-block; width:3px; height:0.85em; background:#F5B800; vertical-align:middle; margin-left:3px; animation:blink 1s step-start infinite; border-radius:1px; }
    ...
    .btn-hero-gold::before { content:''; position:absolute; top:0; left:-100%; width:100%; height:100%; background:linear-gradient(90deg,transparent,rgba(255,255,255,0.35),transparent); transition:left 0.45s; }
    ...
    .feat-card::before { content:''; position:absolute; inset:0; opacity:0; background:radial-gradient(circle at 50% 0%,var(--feat-color,#F5B800),transparent 60%); transition:opacity 0.3s; }
    ...
    .feat-card:hover::before { opacity:0.12; }
    ...
    .stat-card-anim { background:var(--dark-2); border:1px solid var(--border); border-radius:16px; transition:all 0.3s; animation:borderGlow 3s ease infinite; }
    ...
    .scanline { position:absolute; left:0; width:100%; height:2px; background:linear-gradient(90deg,transparent,rgba(245,184,0,0.08),transparent); animation:scanline 6s linear infinite; pointer-events:none; }
  ...
  return React.createElement('div', { style: { paddingTop: 64, overflowX: 'hidden' } },
    ...
    React.createElement('section', {
      ...
      style: { minHeight: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '4rem 1.5rem', position: 'relative', overflow: 'hidden' },
    },
      React.createElement(ParticleCanvas),
      React.createElement('div', { style: { position: 'absolute', inset: 0, pointerEvents: 'none', backgroundImage: 'linear-gradient(rgba(245,184,0,0.035) 1px,transparent 1px),linear-gradient(90deg,rgba(245,184,0,0.035) 1px,transparent 1px)', backgroundSize: '60px 60px' } }),
      ...
      React.createElement('div', { style: { position: 'absolute', width: 800, height: 800, pointerEvents: 'none', background: 'radial-gradient(circle,rgba(245,184,0,0.06) 0%,transparent 65%)', borderRadius: '50%', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' } }),
      React.createElement('div', { style: { position: 'absolute', width: 400, height: 400, pointerEvents: 'none', background: 'radial-gradient(circle,rgba(75,158,232,0.05) 0%,transparent 70%)', borderRadius: '50%', top: '-100px', right: '-100px' } }),

      React.createElement('div', { style: { position: 'relative', maxWidth: 860, zIndex: 1, width: '100%' } },

        // Eyebrow badge
        React.createElement('div', { style: { display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(245,184,0,0.08)', border: '1px solid rgba(245,184,0,0.25)', borderRadius: 50, padding: '0.4rem 1rem', marginBottom: '2rem', opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.6s cubic-bezier(0.16,1,0.3,1)' } },
          ...
          React.createElement('span', { style: { fontFamily: "'Space Mono',monospace", fontSize: '0.7rem', color: '#F5B800', letterSpacing: '3px', textTransform: 'uppercase' } }, "India's #1 Tournament Platform")
        ),

        // TOURNAMENT title
        React.createElement('h1', { style: { fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(4rem,14vw,9.5rem)', lineHeight: 0.88, letterSpacing: '3px', color: '#F0EEE8', marginBottom: '0.2rem', opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0) skewY(0)' : 'translateY(60px) skewY(3deg)', transition: 'all 0.8s cubic-bezier(0.16,1,0.3,1) 0.1s' } }, 'TOURNAMENT'),

        // HUB shimmer
        React.createElement('h1', { style: { fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(4rem,14vw,9.5rem)', lineHeight: 0.88, letterSpacing: '3px', marginBottom: '1.75rem', opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(60px)', transition: 'all 0.8s cubic-bezier(0.16,1,0.3,1) 0.2s' } },
        ...
        // Subtitle
        React.createElement('p', { style: { fontSize: '1.1rem', color: '#8A8A9A', maxWidth: 500, margin: '0 auto 2.5rem', lineHeight: 1.75, opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(30px)', transition: 'all 0.7s cubic-bezier(0.16,1,0.3,1) 0.35s' } },
        ...
        React.createElement('div', { style: { display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.7s cubic-bezier(0.16,1,0.3,1) 0.45s' } },
          user
        ...
        // Floating badges
        React.createElement('div', { style: { display: 'flex', gap: '0.75rem', justifyContent: 'center', marginTop: '2.5rem', flexWrap: 'wrap', opacity: heroVisible ? 1 : 0, transition: 'all 0.7s cubic-bezier(0.16,1,0.3,1) 0.6s' } },
          [
            ...
            React.createElement('div', { key: b.text, style: { display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 50, padding: '0.35rem 0.9rem', fontSize: '0.78rem', fontWeight: 500, color: '#8A8A9A', animation: 'floatUp 0.5s ease forwards', animationDelay: (0.7 + i * 0.08) + 's', opacity: 0 } },
              React.createElement('span', null, b.icon), b.text
            )
          )
        ),
      )
    ...
    React.createElement('section', { style: { padding: '1.5rem 0', overflow: 'hidden', background: 'var(--dark-2)', borderTop: '1px solid rgba(245,184,0,0.1)', borderBottom: '1px solid rgba(245,184,0,0.1)' } },
      React.createElement('div', { style: { display: 'flex', gap: '1rem', animation: 'marquee 25s linear infinite', width: 'max-content', marginBottom: '0.75rem' } },
        [...SPORTS, ...SPORTS].map(([e, n], i) =>
          React.createElement('div', { key: i, className: 'ticker-item', style: { display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: 'var(--dark-3)', borderRadius: 50, border: '1px solid var(--border)', fontSize: '0.82rem', fontWeight: 500, whiteSpace: 'nowrap' } },
      ...
      React.createElement('div', { style: { display: 'flex', gap: '1rem', animation: 'marqueeRev 30s linear infinite', width: 'max-content' } },
        [...SPORTS.slice().reverse(), ...SPORTS.slice().reverse()].map(([e, n], i) =>
          React.createElement('div', { key: i, className: 'ticker-item', style: { display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: 'var(--dark-3)', borderRadius: 50, border: '1px solid var(--border)', fontSize: '0.82rem', fontWeight: 500, whiteSpace: 'nowrap', opacity: 0.7 } },
            React.createElement('span', { style: { fontSize: '1rem' } }, e), n
          )
        )
      )
    ...
    React.createElement('section', { ref: statsRef, style: { padding: '4rem 1.5rem', borderBottom: '1px solid var(--border)' } },
      React.createElement('div', { className: 'container' },
        React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: '1rem' } },
          STATS.map((s, i) =>
            React.createElement('div', { key: s.label, className: 'stat-card-anim', style: { animationDelay: (i * 0.5) + 's', opacity: statsVis ? 1 : 0, transform: statsVis ? 'translateY(0)' : 'translateY(30px)', transition: 'opacity 0.6s ease ' + (i * 0.12) + 's,transform 0.6s ease ' + (i * 0.12) + 's' } },
    ...
    React.createElement('section', { ref: featRef, style: { padding: '5rem 1.5rem' } },
      React.createElement('div', { className: 'container' },
        React.createElement('div', { style: { textAlign: 'center', marginBottom: '3rem', opacity: featVis ? 1 : 0, transform: featVis ? 'translateY(0)' : 'translateY(30px)', transition: 'all 0.7s ease' } },
          React.createElement('h2', { style: { fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2rem,5vw,3rem)', letterSpacing: 2 } }, 'Everything You Need'),
          React.createElement('p', { style: { color: '#8A8A9A', maxWidth: 460, margin: '0.5rem auto 0' } }, 'Powerful tools for hosts and players. Zero complexity.')
        ),
        React.createElement('div', { className: 'grid-3' },
          FEATURES.map((f, i) =>
            React.createElement('div', { key: f.title, className: 'feat-card', style: { '--feat-color': f.color, opacity: featVis ? 1 : 0, transform: featVis ? 'translateY(0)' : 'translateY(40px)', transition: 'opacity 0.6s ease ' + (i * 0.1) + 's,transform 0.6s ease ' + (i * 0.1) + 's' } },
              ...
              React.createElement('div', { style: { height: 2, background: f.color, borderRadius: 2, marginTop: '1rem', opacity: 0.3 } })
            )
          )
        )
      )
    ...
    // ── CTA ───────────────────────────────────────────────────
    !user && React.createElement('section', { ref: ctaRef, style: { padding: '5rem 1.5rem', textAlign: 'center', background: 'var(--dark-2)', borderTop: '1px solid var(--border)', position: 'relative', overflow: 'hidden' } },
      ...
      React.createElement('div', { className: 'container', style: { position: 'relative' } },
        React.createElement('h2', { style: { fontFamily: "'Bebas Neue',sans-serif", fontSize: 'clamp(2.5rem,6vw,4rem)', letterSpacing: 2, marginBottom: '1rem', opacity: ctaVis ? 1 : 0, transform: ctaVis ? 'translateY(0)' : 'translateY(30px)', transition: 'all 0.7s ease' } }, 'Ready to Compete?'),
        React.createElement('p', { style: { color: '#8A8A9A', fontSize: '1.05rem', maxWidth: 400, margin: '0 auto 2.5rem', opacity: ctaVis ? 1 : 0, transition: 'all 0.7s ease 0.1s' } }, 'Join as a Player to enter tournaments, or as a Host to run your own events.'),
        React.createElement('div', { style: { display: 'flex', gap: '1rem', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap', opacity: ctaVis ? 1 : 0, transition: 'all 0.7s ease 0.2s' } },
